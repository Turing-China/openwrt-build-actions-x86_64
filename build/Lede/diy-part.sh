...
# 后台IP设置
export Ipv4_ipaddr="192.168.1.58"            # 修改openwrt后台地址(填0为关闭)
...
version.buildinfo
profiles.json
openwrt-x86-64-$(TZ=UTC-8 date "+%Y.%m.%d")-generic-kernel.bin
openwrt-x86-64-$(TZ=UTC-8 date "+%Y.%m.%d")-generic.manifest
openwrt-x86-64-$(TZ=UTC-8 date "+%Y.%m.%d")-generic-squashfs-rootfs.img.gz
EOF
